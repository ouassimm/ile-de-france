...
def execute(context):
    ...
    df = pd.read_excel(
        "%s/filosofi_2015/FILO2018_DISP_COM.xls" % context.config("data_path"),
        sheet_name="ENSEMBLE", skiprows=5
    )[["CODGEO"] + ["D%d18" % q if q != 5 else "Q218" for q in range(1, 10)]]
    ...
    return df[["commune_id", "q1", "q2", "q3", "q4", "q5", "q6", "q7", "q8", "q9", "is_imputed", "is_missing",
               "reference_median"]]


def validate(context):
    if not os.path.exists("%s/filosofi_2015/FILO2018_DISP_COM.xls" % context.config("data_path")):
        raise RuntimeError("Filosofi data is not available")

    return os.path.getsize("%s/filosofi_2015/FILO2018_DISP_COM.xls" % context.config("data_path"))
